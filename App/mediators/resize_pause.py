"""Pause matplotlib + OpenGL redraws while the main app shell is being resized."""
...
def end_resize_pause(app) -> None:
    ...
    if getattr(app, "_pending_canvas_draw", False):
        ...
        glue = getattr(app, "layout_glue", None)
        if glue is not None and getattr(glue, "_relayout_plots", None) is not None:
            glue._plots_layout_size = None
            glue._relayout_plots()
            return
...
def attach_resize_pause(app, panes, watch_widgets=(), debounce_ms=_DEBOUNCE_MS) -> None:
    ...
    app._resize_configure_handler = _on_configure
    app._resize_configure_state = state


def register_resize_watch_widgets(app, widgets) -> None:
    """Attach configure-based resize pause to widgets created after startup."""
    handler = getattr(app, "_resize_configure_handler", None)
    if handler is None:
        return
