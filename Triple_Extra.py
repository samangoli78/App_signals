...
class Triple_Extra():
    ...
    def find_windows(self,ax,stiulation,refference,margin=0):
        ...
        P,dic=find_peaks(stiulation,distance=200,height=0.3*stiulation.max())
        ...
        P1,dic=find_peaks(ref,distance=500,height=0.5*refference.max())
        self.sinus_ref=P1
        P1=P1
        ax.plot(P1*0.001,dic["peak_heights"],"x")
        ref=refference[self.stim_start[-1]+self.stim_duration[-1]:]
        PP,dic=find_peaks(ref,distance=500,height=0.5*refference.max())
